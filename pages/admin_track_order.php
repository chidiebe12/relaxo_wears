...
$statusSteps = ['pending','approved','shipped','delivered']; // show shipped instead of completed
...
<html>
<head>
<title>Admin Track Order #<?= $order_id ?></title>
...
.status-step.active .step-circle{background:#2d6a4f;}
...
<p><strong>Drop-off Location:</strong> <?= htmlspecialchars($order['drop_address'] ?? 'Not set') ?></p>
</div>
...
.bindPopup("📍 <?= addslashes($order['drop_address'] ?? 'Drop-off not set') ?>")
.openPopup();
</script>
</body>
</html>
